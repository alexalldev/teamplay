...
function GamePlaySockets(socket, io) {
      ...
      GameTeam.findAll({ raw: true, where: { Game_Id: session.Game.Id } })
        .then(gameTeams => {
          if (gameTeams.length > 0) {
            ...
            } else socket.emit("ReturnPrepareGame", false);
          } else socket.emit("ReturnPrepareGame", false);
        })
  ...
  socket.on("GetPrepared", function() {
    // Команда запрашивает подготовленность игры при входе в игру
    if (session.Game) {
                ...
                GamePlayCategory.findAll({
                  ...
                  .then(async gamePlayCategories => {
                    if (gamePlayCategories.length > 0) {
                      let count = 0;
                      for (let gamePlayCategory of gamePlayCategories) {
                        await GamePlayQuestion.findOne({
                          ...
                          .then(async gamePlayQuestion => {
                            if (gamePlayQuestion) await count++;
                          })
                          .catch(err => console.log(err));
                      }
                      if ((await count) == 0) {
                            ...
                            await new SelectionTimer(
                              ...
                              function(Stimer) {
                                ...
                                new AnsweringTimer(
                                  ...
                                  Stimer,
                                  io
                                ); // Добавляем ссылку на Stimer, чтобы его не искать
                              },
                              io
                            );
                            await io
                              .to(`Teams${session.Game.Id}`)
                              .emit("StartGame");
                            socket.emit("ReturnStartGame", true);
                          }
                        })
                        .catch(err => console.log(err));
                    } else socket.emit("ReturnStartGame", "null_categories");
                  })
                  .catch(err => console.log(err));
              }
            })
  ...
  socket.on("GetCategoriesList", function() {
    // Команда запрашивает список вопросов
                  ...
                  if (gamePlayCategories.length > 0) {
                    ...
                    for (var C in gamePlayCategories) {
                      await Category.findOne({
                        raw: true,
                        where: { CategoryId: gamePlayCategories[C].Category_Id }
                      })
                        .then(category => {
                      ...
                      await GamePlayQuestion.findAll({
                        ...
                        where: {
                          GamePlayCategory_Id:
                            gamePlayCategories[C].GamePlayCategoryId
                        }
                      })
                          ...
                          if (gamePlayQuestions.length > 0) {
                            ...
                            for (var Q in gamePlayQuestions) {
                              await Question.findOne({
                                raw: true,
                                where: {
                                  QuestionId: gamePlayQuestions[Q].Question_Id
                                }
                              })
                                .then(async question => {
                                  question.GamePlayQuestionId =
                                    gamePlayQuestions[Q].GamePlayQuestionId;
  ...
  socket.on("TeamSelectQuestion", function(gamePlayQuestionId) {
    // Команда выбирает вопрос
    if (session.Game) {
      GameTeam.findOne({
        raw: true,
        where: { GameTeamId: session.Game.GameTeamId, CanSelect: true }
      })
        .then(gameTeam => {
          if (gameTeam) {
            GamePlayQuestion.findOne({
              where: { GamePlayQuestionId: gamePlayQuestionId }
            })
              .then(gamePlayQuestion => {
                Question.findOne({
                  raw: true,
                  where: { QuestionId: gamePlayQuestion.dataValues.Question_Id }
                })
                  .then(question => {
                    if (question) {
                      ...
                      GamePlayQuestion.destroy({
                        where: {
                          GamePlayQuestionId:
                            gamePlayQuestion.GamePlayQuestionId
                        }
                      });
  ...
  socket.on("AdminStopGame", function() {
    // Администратор завершает игру
    if (socket.HasControlGame) {
  ...
  function StopGame(gameId) {
    GamePlay.findOne({ where: { Game_Id: gameId } }).then(gamePlay => {
      if (gamePlay) {
        ...
        GameTeam.findAll({
          ...
          .then(async gameTeams => {
            ...
            for (var gameTeam of gameTeams) {
              await Team.findOne({
                raw: true,
                where: { TeamId: gameTeam.dataValues.Team_Id }
              }).then(team => {
                Teams.push({
  ...
  socket.on("GetCanSelect", function() {
    // Команда запрашимает значение возможности выбора вопроса
    if (session.Game)
      GameTeam.findOne({
        raw: true,
        where: { GameTeamId: session.Game.GameTeamId }
        ...
        .then(gameTeam => {
          if (gameTeam) socket.emit("ReciveCanSelect", gameTeam.CanSelect);
        })
        .catch(err => console.log(err));
  });

  socket.on("GetPoints", function() {
    // Команда запрашимает количество очков
    if (session.Game)
      GameTeam.findOne({
        raw: true,
        where: { GameTeamId: session.Game.GameTeamId }
        ...
        .then(gameTeam => {
          if (gameTeam) socket.emit("RecivePoints", gameTeam.Points);
        })
        .catch(err => console.log(err));
  });
  ...
  socket.on("TeamAnswer", function(data) {
    ...
    if (session.Game) {
      GameTeam.findOne({
        where: { GameTeamId: session.Game.GameTeamId, CanAnswer: true }
      })
        .then(async gameTeam => {
          if (gameTeam) {
            gameTeam.update({ CanAnswer: false }); // Запрещаем отвечать второй раз

            let flag = answers.length > 0;
            for (var A in answers) {
              await Answer.findOne({
                raw: true,
                where: { AnswerId: answers[A].AnswerId }
              })
                .then(answer => {
                  if (answer) {
                    if (answers[A].Value != answer.Correct) flag = false;
                  } else flag = false;
                })
                .catch(err => {
                  console.log(err);
            ...
            await Question.findOne({
              ...
              .then(async question => {
                if (question) {
                  let TempPoints = (await flag) ? question.QuestionCost : 0;
                  gameTeam.update({ TempPoints, Answered: true }).then(() => {
                    socket.emit("AnswerSaved");
                    GameTeam.findAll({
                      raw: true,
                      where: { Game_Id: session.Game.Id, Answered: false }
                    }).then(gameTeams => {
                      if (gameTeams.length == 0)
                        GetTimer(
                          session.Game.Id,
  ...
  socket.on("TeamSingleAnswer", function(text) {
    // Команда отвечает на текстовый вопрос
    if (session.Game)
      if (session.Team) {
        GameTeam.findOne({
          where: { GameTeamId: session.Game.GameTeamId, CanAnswer: true }
          ...
          .then(gameTeam => {
            if (gameTeam) {
              ...
              io.to(`Admins${session.Game.Id}`).emit("AddSingleAnswer", {
                Team: session.Team,
                GameTeamId: session.Game.GameTeamId,
                Text: text
              });
            }
          })
  ...
  socket.on("UpdateCorrectAnswer", function(data) {
    // Администратор отмечает правильность текстового ответа
    if (socket.HasControlGame) {
      const { GameTeamId, QuestionCost, Correct } = data;

      GameTeam.findOne({ where: { GameTeamId } })
        .then(gameTeam => {
          if (gameTeam) {
  ...
  socket.on("ConfrimSingleAnswers", function() {
    // Администратор завершает проверку текстовых ответов
    if (socket.HasControlGame) {
  ...
  function PrepareGame() {
    // Подготовка игры с Id из session
    if (socket.HasControlGame) {
      ...
      Game.findOne({ raw: true, where: { GameId: session.Game.Id } })
        .then(game => {
          GameTeam.findAll({ where: { Game_Id: session.Game.Id } })
            .then(async gameTeams => {
              let RandomIds = GetRandomIds(gameTeams.length);
              for (var G in gameTeams) {
                await gameTeams[G].update({
                  Play: 1,
                  TempPoints: 0,
                  Answered: 0,
                  Points: 0,
                  ...
                  .then(async gameTeam => {
                    await io.emitTeam(
                      gameTeam.dataValues.GameTeamId,
                      "PrepareGame",
                      {
                        PlayId: RandomIds[G],
                        SelectionTime: game.SelectionTime,
      ...
      GamePlay.findOne({ where: { GamePlayId: session.Game.Id } })
        .then(gamePlay => {
          if (gamePlay) gamePlay.update({ StartTime: 0, StopTime: 0 });
        })
        .catch(err => console.log(err));
                  ...
                  if (categories.length > 0)
                    for (var C in categories) {
                      await GamePlayCategory.findOrCreate({
                        raw: true,
                        where: {
                          Category_Id: categories[C].CategoryId,
                        ...
                        .then(async ([gamePlayCategory, created]) => {
                          await Question.findAll({
                            raw: true,
                            where: { Category_Id: categories[C].CategoryId }
                          })
                            .then(async questions => {
                              for (let Q in questions) {
                                await GamePlayQuestion.findOrCreate({
                                  ...
                                  .then(
                                    async ([gamePlayQuestion, created]) => {}
                                  )
                                  .catch(err => console.log(err));
                              }
                            })
                  ...
                  .then(question => {
                    if (question) {
                      GamePlay.findOne({ where: { Game_Id: game.GameId } })
                        .then(gamePlay => {
                          if (gamePlay) {
                            io.to(`Teams${session.Game.Id}`).emit(
                              "StartAnswerQuestion",
                              answers.length == 1
                            ...
                            gamePlay
                              .update({ CurrentQuestionId: questionId })
                              .then(() => {
                                io.to(`Stream${session.Game.Id}`).emit(
                                  "ReciveQuestion",
                                  question
                                );
                              });
...
function GetRandomIds(Count) {
  ...
  for (let c = 1; c <= Count; c++) RandomNumbers.push(c);
...
async function GetTimer(gameId, callback, type = TIMERS.Types.Selection) {
  for (let TIMER of TIMERS[type]) {
    if (TIMER.GameId == gameId) return callback(TIMER);
  }
  await callback(null);
}

function ClearTimers(gameId) {
  for (const Ts in TIMERS.Selection) {
    if (TIMERS.Selection[Ts].GameId == gameId) {
      TIMERS.Selection[Ts].Clear();
      TIMERS.Selection.splice(Ts, 1);
    }
  }
  for (const Ta in TIMERS.Answering) {
...
function SelectionTimer(gameId, callback, io) {
  let Timer = null;
  if (!io) console.error("Selection_Timer_ERROR: io is undefined");
  let GameTeams = [];
  let currentGameTeamIndex = 0;
  let currentGameTeam = null;
  Game.findOne({ raw: true, where: { GameId: gameId } })
    .then(game => {
      if (game) {
        let SelectionTime = game.SelectionTime * 1000;
        GameTeam.findAll({
          raw: true,
          order: [["PlayId", "ASC"]],
          where: { Game_Id: game.GameId }
          ...
          callback({
            ...
            Stop() {
              ChangeGameTeamsState(
                ...
                io
              ); //Меняем состояние команд на ответ
            },
            Start() {
              ChangeGameTeamsState(
                gameId,
                "1",
                function(state) {
                  //Меняем состояние команд на выбор вопроса
                  if (state) {
                    Tick(remaining);
  ...
  var Tick = async function(remaining) {
    //* ****Команда прошлая */
    if (currentGameTeam) {
      await GameTeam.findOne({ where: { GameTeamId: currentGameTeam } }).then(
        async gameTeam => {
          if (gameTeam) {
    ...
    } else io.to(`Teams${gameId}`).emit("WaitSelect");
    //* ****Команда прошлая */

    await io.emitTeam(
      GameTeams[currentGameTeamIndex],
      "SelectQuestion",
      remaining
    );
    ...
    await GameTeam.findOne({ where: { GameTeamId: currentGameTeam } })
      .then(async gameTeam => {
        if (gameTeam)
          await gameTeam.update({ CanSelect: true }).then(async () => {
            currentGameTeamIndex =
              (await currentGameTeamIndex) == GameTeams.length - 1
                ? 0
                : ++currentGameTeamIndex;
          });
      })
    ...
    .then(async gameTeams => {
      if (gameTeams.length > 0) {
        ...
        GamePlay.findOne({ where: { Game_Id: gameId } }) // Меняем состояние игры для зрителей
          .then(gamePlay => {
            gamePlay.update({ StreamState: state != 1 });
          })
          .catch(err => console.log(err));

        if (state == 2)
          io.to(`Teams${gameId}`).emit("CantSelectQuestion", true);
        if (state == 1) io.to(`Teams${gameId}`).emit("CantAnswer");
      }
    })
    .catch(err => {
      console.log(err);
...
function AnsweringTimer(gameId, callback, Stimer, io) {
  let Timer = null;
  if (!io) console.error("Answering_Timer_ERROR: io is undefined");
  if (!Stimer) console.error("Answering_Timer_ERROR: Stimer is undefined");
  Game.findOne({ raw: true, where: { GameId: gameId } })
    .then(game => {
      if (game) {
  ...
  var Stop = function(pause = false) {
    // Останавливает таймер ответа и записывает полученные баллы
    clearInterval(Timer);
    if (pause == false) {
      ContinueSelection(
        gameId,
        ...
        for (let gameTeam of gameTeams) {
          if (gameTeam.dataValues.Answered) {
            io.emitTeam(
              gameTeam.dataValues.GameTeamId,
              "AddPoints",
              gameTeam.dataValues.TempPoints
            ); // Добавить очки команде, ответившей на вопрос
          }
          await gameTeam.update({
            Points: gameTeam.dataValues.Points + gameTeam.dataValues.TempPoints,
            Answered: false
          });
        }
