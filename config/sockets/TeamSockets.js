...
module.exports = function(socket, io) {
	...
	socket.on('CreateTeam', function(TeamName) {
		if (session.passport.user) {
			if (TeamName) {
				if (TeamName.charAt(TeamName.length - 1) == ' ') TeamName = TeamName.substr(0, TeamName.length - 1);
				TeamTag = TeamName.replace(/[^a-zA-Zа-яА-Я0-9 ]/g, '').toLowerCase().replace(/\s/g, '-');

				User.findOne({ where: { UserId: session.passport.user } }).then((user) => {
					if (user)
						if (user.dataValues.Team_Id == 0)
							Team.findOrCreate({ where: { TeamTag: TeamTag } })
								.then(([ team, created ]) => {
									if (created == true) {
										team
											...
											.then(() => {
												user
													.update({ isCoach: true, Team_Id: team.dataValues.TeamId })
								...
								.catch((err) => console.log(err));
						else socket.emit('Info', 'Покиньте текущую команду, чтобы создать новую');
				});
			}
		}
	});
			...
			User.findOne({ where: { UserId: session.passport.user } }).then((Me) => {
				if (Me)
					Team.findOne({ where: { TeamId: Me.Team_Id } }).then((team) => {
						if (team) {
							User.findOne({ where: { Team_Id: Me.Team_Id, isCoach: 1 } }).then((coach) => {
								if (Me.isCoach)
									User.count({ where: { Team_Id: team.TeamId } }).then((result) => {
										console.log(result);
										if (result == 1) {
											Team.destroy({ where: { TeamId: team.TeamId } }).then(() => {
												Me.update({ Team_Id: 0, isCoach: false }).then(() => {
										...
										notification(
											{
												...
												senderId: Me.UserId,
												header: 'Моя команда',
												mainText: `Пользователь ${Me.UserFamily} ${Me.UserName.slice(
													0,
													1
												)}. ${Me.UserLastName.slice(0, 1)}. покинул вашу команду`,
