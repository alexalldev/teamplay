socket.on('receiveNotification', function(result) {
	...
	От: ${notification.userTeam && notification.userTeam != 0
	 ...
	 <div class="nav-notification">
		...
		if (result.createdNotification.addToStart) $('.notifications-menu').prepend(ringNotification);
...
function getAnswer(InvitationType, answer) {
	switch (InvitationType) {
		...
		case 'sentBefore':
			return answer == 'inviteTeam' ? 'Вы уже приглашали этого игрока' : 'Вы уже подали заявку в эту команду';
	}
}
