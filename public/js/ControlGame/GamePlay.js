...
async function PrepareGame() {
	...
	if ($('i').is('.VerifyTeam')) {
		...
		if (flag == true) socket.emit('PrepareGame');
		else Swal('Верификация', 'Не все команды верифицированы. Можно удалить неверифицированные и начать игру.', 'warning');
	} else Swal('Нет команд', 'Подождите, пока игроки войдут в игру.', 'warning');
}
...
function PrepareTeams() {
	$('.card').each(function(i) {
		$(this).delay(i * 100).animate({ opacity: 0 }, { queue: true, duration: 100 });
	});
	setTimeout(() => {
		$('.TeamsList').remove();
...
socket.on('ReturnStartGame', function(started) {
	if (started == true) Swal('Отлично!', 'Игра начата', 'success');
	else if (started == 'null_questions') Swal('Игра не начата', 'В этой игре нет вопросов', 'info');
	else if (started == 'null_categories') Swal('Игра не начата', 'В этой игре нет категорий', 'info');
});

socket.on('AddTeamPoints', async function(data) {
	//Сортировка при каждом получении баллов
    ...
    <td>\
        <div class="d-flex align-items-center justify-content-center h-1">\
            <label class="Checkcontainer text-center mb-4 ml-1">\
                <input type="checkbox" onchange="UpdateCorrectAnswer(' +
			GameTeamId +
			', $(this).prop(\'checked\'))">\
