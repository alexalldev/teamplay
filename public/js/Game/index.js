...
function Verificate(state) {
    if (state)
        $('.VerificationText').text('Команда верифицирована');
    else
        $('.VerificationText').text('Заявите о своем присутствии');
...
socket.on('PrepareGame', function (GameParams) {
    ...
    ShowInfo("Подготовка к началу игры", ["На выбор вопроса предоставляется " + SelectionTime + " с",
    "На ответ предоставляется " + AnswerTime + " с", "___",
    "Ваша команда выбирает вопрос " + PlayId + "-ой"], true);
});
...
function ShowInfo(Header, fields, loader = false) {
    ...
    fields.forEach(field => {
        $('.InfoText').html($('.InfoText').html() + '<tr align="center">\
        <td valign="top">\
          <span class="h2">'+ field + '</span>\
        </td>\
      </tr>');
    });
    $('.Info').attr('show', 'true');
...
socket.on('ReciveCategoriesList', async function (Categories) {
    ...
    for (var C in Categories) {
        if (Categories[C].Questions) {
            ...
            <span class="h2 CategoryName">'+ Categories[C].CategoryName + '</span>\
            <table class="table table-hover">\
                <tr>';
            Categories[C].Questions = await bubbleSort(Categories[C].Questions);
            for (var Q in Categories[C].Questions) {
                insertText += '<td class="Question-Item" onclick="(SelectQuestion(' + Categories[C].Questions[Q].GamePlayQuestionId + '))">' + Categories[C].Questions[Q].QuestionCost + '</td>';
            }
            await $('.CategoriesList').html($('.CategoriesList').html() + insertText + '</tr>\
            </table>\
            </div>\
        </div>');
        }
...
function AddPoints(count, set = false) {
    ...
    animateCSS('.AddPoints', 'slideInUp', function () {
        ...
        $('.PointsCount').text(!set ? (Number($('.PointsCount').text()) + count) : (count));
    });
}

socket.on('ReciveCanSelect', function (state) {
    if (state) {
        InfoLabel('Похоже, вы перезагрузили страницу. Выбирайте вопрос, у вас еще есть время.');
...
function LoadAnswers(answers) {
    var insertText = '<div class="row">\
        <div class="col-md-12 text-center">\
        <form action="javascript:void(null)" onsubmit="Answer('+ answers[0].Question_Id + ')">\
            <div class="form-group">\
            <table style="width: 100%">';
    for (var answer of answers) {
        insertText += '<tr>\
                    <td><span class="h2">'+ answer.AnswerText + '</span></td>\
                    <td>\
                        <div class="d-flex align-items-center justify-content-center h-1">\
                            <label class="Checkcontainer text-center mb-4 ml-1">\
                                <input type="checkbox" class="Answer-'+ answer.Question_Id + ' AnswerCheck-' + answer.AnswerId + '" AnswerId="' + answer.AnswerId + '">\
...
function LoadAnswer(questionId) {
    $('.AnswersListContainer').html('<div class="row">\
    <div class="col-md-12 text-center">\
        <form action="javascript:void(null)" onsubmit="Answer('+ questionId + ')">\
            <div class="form-group">\
