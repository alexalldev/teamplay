    ...
    $.ajax({
      ...
      success: function(data) {
        if (data == "incorrect_email") $("#email").addClass("incorrect-input");
          ...
          $("#Info").html(
            "Позаботьтесь о своей безопасности, придумав пароль длинной от шести символов."
          );
          ...
          AnimateMessage(
            "Отлично, что вы с нами. Проверьте почту для подтверждения."
          );
        }
      },
...
function WelcomeUrl(UserName) {
  if (!window.WelcomeUrlTimer) {
    ...
    window.history.state.message != window.WelcomeMessage
  ) {
