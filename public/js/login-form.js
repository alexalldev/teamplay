    ...
    $.ajax({
        ...
        success: function (data) {
            if (data == 'pass')
                ...
                $("#Message").text('Похоже такого пользователя не существует');
            ...
            else if (JSON.parse(data).result)
...
function WelcomeUrl(UserName) {
    if (!window.WelcomeUrlTimer) {
        ...
        if (!window.history.state || window.history.state.message != window.WelcomeMessage) {
            window.history.replaceState({ message: window.WelcomeMessage.substr(0, window.WelcomeCounter) }, '', window.WelcomeMessage.substr(0, window.WelcomeCounter));
