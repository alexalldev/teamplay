...
$(".btnChangePassword").click(function() {
  Swal.fire({ title: "Текущий пароль", input: "password" }).then(
    currentpass => {
      if (currentpass.value)
        Swal.fire({
          title: "Новый пароль",
          text: "Длина пароля не менее 5 символов",
          input: "password"
        }).then(newpass => {
          if (newpass.value)
            $.ajax({
              ...
              success: function(data) {
                if (data == "incorrect_pass")
                  Swal.fire("Неверный текущий пароль", "", "warning");
                if (data == "short_pass")
                  Swal.fire("Длина пароля не менее 5 символов", "", "warning");
                if (data == "need_pass")
                  Swal.fire("Введите оба пароля ", "", "warning");
