...
socket.on('recieveRoomPlayers', async function(players) {
...
socket.on('NewRoomGroupCoach', function(roomPlayer) {
    ...
    $('.isGroupCoach-' + roomPlayer.RoomPlayersId).html('<span style="position: absolute; left:90%; bottom:30%"><i class="fas fa-star text-warning"></i></span>')
});
...
function AddPlayerToGroup(player) {
    $('.group-players-' + player.TeamId).append('<div class="row Player Player-'+ player.RoomPlayersId +' mt-2 mb-2">\
        <div class="col-md-12 text-left">\
        <img class="user_organizer" src="/public/user-icon.png" alt="">\
        <span style="font-size: 30px; position:relative; top: 5%"> '+ player.UserFamily + ' ' + player.UserName.slice(0, 1) + '.' + player.UserLastName.slice(0, 1) + '.</span>\
        <span class="isGroupCoach-'+ player.RoomPlayersId +'">'+ (player.isGroupCoach ? '<span style="position: absolute; left:90%; bottom:30%"><i class="fas fa-star text-warning"></i></span>' : '') +'\
        </div>\
    </div>');
}

async function CreateGroup(player) {
    $('.group-list').append('<div class="group group-'+ player.TeamId +' col-md-4" TeamId="'+ player.TeamId +'">\
