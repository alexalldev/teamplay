...
app.use(function(req, res, next) {
	if (req.session.roomId) {
		...
		else if (!req.path.toLowerCase().includes('room/')) {
			Room.findOne({ where: { RoomId: req.session.roomId } })
				.then(room => {
					if (room)
						RoomPlayer.findOne({ where: { Room_Id: room.RoomId }, raw: true }).then(roomPlayer => {
							if (roomPlayer) return res.render('info', { message: 'LEAVE_ROOM', room: room });
							else return next();
						});
					else {
