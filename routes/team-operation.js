...
router.post("/invite", urlencodedParser, async function(req, res) {
  ...
  let reciever_user;
  await User.findOne({
    where: { UserId: req.body.receiverId, UserIsActive: true },
    raw: true
  })
    .then(userReceiver => {
      ...
      .then(sender_user => {
        ...
        if (req.body.isInfoNotification)
          if (
            ...
            ((!sender_user.isCoach || reciever_user.Team_Id != 0) &&
  ...
  if (
    (req.body.shouldCreate == "true" && canSend) ||
    req.body.shouldCreate == "false"
  ) {
    notification(
      {
  ...
  Team.findOne({ where: { TeamName: req.body.oldName } })
    .then(team => {
      if (!team) {
        ...
        Team.update(
          { TeamName: req.body.newName },
          { where: { TeamName: req.body.oldName } }
        )
          .then(team => {
            res.send(true);
          })
...
router.post("/changeCoach", urlencodedParser, function(req, res) {
  User.findOne({
    where: { UserId: req.body.oldCoachId, Team_Id: req.body.teamId }
  })
    .then(old => {
      if (!old) {
        res.send(false);
        ...
        User.findOne({
          where: { UserId: req.body.newCoachId, Team_Id: req.body.teamId }
        })
          .then(newCap => {
            if (!newCap) {
              res.send(false);
